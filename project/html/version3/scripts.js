...
function init() {
	...
	let login3 = ["Woomin", "1234"];
	let login4 = ["Owen", "1234"];

	normal_creds.push(login1);
	normal_creds.push(login2);
	normal_creds.push(login3);
	admin_creds.push(login4);
...
function delete_user(user) {
	...
	document.getElementById("user_table").removeChild(user);
	return;
}
...
function create_user_list() {
	...
	for(let i=0;i<normal_creds.length;i++) {
		create_user_table_element(normal_creds[i][0]);
	}
}
...
function create_user_table_element(user) {
	...
	td1.innerHTML = td1.innerHTML + user;
	tr.appendChild(td1);
	
	var td2 = document.createElement("td");
	td2.appendChild(create_delete_button(user));
	tr.appendChild(td2);
